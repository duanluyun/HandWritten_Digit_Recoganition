...
def inference(input_tensor,train,regularizer):
    ...
    with tf.variable_scope('layer5-fc1',reuse=tf.AUTO_REUSE):
        ...
        fc1_baises=tf.get_variable("bias",[FC_SIZE],initializer=tf.constant_initializer(0.1))
        fc1=tf.nn.relu(tf.matmul(reshape,fc1_weights)+fc1_baises)
        if train:
    ...
    with tf.variable_scope('layer6-fc2',reuse=tf.AUTO_REUSE):
        ...
        fc2_baises=tf.get_variable("bias",[NUM_LABELS],initializer=tf.constant_initializer(0.1))
